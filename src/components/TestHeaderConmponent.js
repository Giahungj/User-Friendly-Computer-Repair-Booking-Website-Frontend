...
import { Button, Box, Badge, Menu, MenuItem, Avatar } from '@mui/material';
import NotificationsNoneIcon from '@mui/icons-material/NotificationsNone';
import LogoutIcon from '@mui/icons-material/Logout';
import { AccountCircleOutlined, HistoryOutlined, EventNoteOutlined } from '@mui/icons-material';
		...
		<>
			<Button onClick={(e) => setAnchorElNoti(e.currentTarget)}>
				<Badge badgeContent={unreadCount > 9 ? '9+' : unreadCount} color="error">
					<NotificationsNoneIcon sx={{ color: '#2196f3' }} />
				</Badge>
			</Button>
			...
			<Menu anchorEl={anchorElUser} open={Boolean(anchorElUser)} onClose={() => setAnchorElUser(null)}>
				...
				<MenuItem onClick={() => handleNavigate(`/profile/${auth.user.email}`)}>
					<AccountCircleOutlined sx={{ mr: 1 }} /> Thông tin cá nhân
				</MenuItem>
				<MenuItem onClick={() => handleNavigate('/bookings/history')}>
					<HistoryOutlined sx={{ mr: 1 }} /> Lịch sử đặt lịch
				</MenuItem>
				<MenuItem onClick={() => handleNavigate('/bookings/upcoming')}>
					<EventNoteOutlined sx={{ mr: 1 }} /> Lịch hẹn sắp tới
				</MenuItem>
				<hr className="m-0" />
				<MenuItem onClick={() => { setAnchorElUser(null); logoutContext(); }} sx={{ fontWeight: 'bold', color: '#ef5350' }}>
